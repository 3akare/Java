...
public class PortServiceMappingScheduler {
    ...
    public PortServiceMappingScheduler(PortServiceMappingRepository portServiceMappingRepository,
            PortServiceMappingEmailSender portServiceMappingEmailSender, Map<String, String> downBad) {
    ...
    public void cron() throws Exception {
        ...
        for (PortServiceMapping portServiceMapping : portServiceMappingList) {
            try (Socket socket = new Socket()) {
                socket.connect(new InetSocketAddress(portServiceMapping.getServiceIpAddress(),
                        portServiceMapping.getServicePortNumber().intValue()), 1000);
    ...
    public void cronEmail() {
        ...
        for (Map.Entry<String, String> entry : downBad.entrySet()) {
            if (Objects.equals(entry.getValue(), "unsent")) {
                PortServiceMapping portServiceMapping = portServiceMappingRepository
                        .findById(Long.parseLong(entry.getKey())).get();
